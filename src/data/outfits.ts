/*
 * File: src/data/outfits.ts
 * Purpose: Large mock dataset of outfits for the web app with categories, styles, and galleries.
 * Notes:
 * - Used by HomePage and ImageViewer; mirrors a subset under mobile/src/shared/ for RN.
 */
import { Outfit } from '../types';

// Generate 1000+ outfits with diverse categories and styles
...
// Post-process: assign unique images, galleries, and extra details to all items
const materials = ['Cotton', 'Linen', 'Wool Blend', 'Silk', 'Denim', 'Polyester', 'Rayon'];
const sizeSets: string[][] = [
  ['XS','S','M','L','XL'],
  ['S','M','L'],
  ['28','30','32','34','36']
];
...
outfits.forEach((item) => {
  // Unique gallery per item using picsum seed
  const gallery = [
    `https://picsum.photos/seed/${item.id}-a/1200/800`,
    `https://picsum.photos/seed/${item.id}-b/1200/800`,
    `https://picsum.photos/seed/${item.id}-c/1200/800`
  ];
  item.images = gallery;
  item.image = gallery[0];

  // Extra details
  item.rating = ((item.id % 5) + 1); // pseudo-rating 1-5
  item.sizes = sizeSets[item.id % sizeSets.length];
  item.material = materials[item.id % materials.length];
  item.sku = `SKU-${item.category.substring(0,3).toUpperCase()}-${item.id.toString().padStart(4, '0')}`;
});

export const categories = ['All', 'Casual', 'Business', 'Evening', 'Seasonal', 'Street', 'Activewear', 'Bohemian'];
export const styles = ['All', 'Relaxed', 'Professional', 'Elegant', 'Sporty', 'Edgy', 'Minimalist', 'Bohemian', 'Vintage', 'Trendy'];
export const seasons = ['All', 'Spring', 'Summer', 'Fall', 'Winter', 'All Season'];
export const occasions = ['All', 'Everyday', 'Work', 'Party', 'Date', 'Beach', 'Gym', 'Festival', 'Meeting', 'Vacation'];
export const colors = ['All', 'Black', 'White', 'Gray', 'Navy', 'Blue', 'Red', 'Pink', 'Green', 'Yellow', 'Brown', 'Beige'];